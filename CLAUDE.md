...
- **`JailbreakBounty`'s payable `fund()` path is now confirmed end-to-end via `gltest`** (2026-07-02, closing half of what was previously listed under "Still open" below) -- `gltest`'s `.transact(value=N)` can send real payable value where the `genlayer` CLI cannot (see the CLI limitation still listed below). `test_funding_accumulates` confirms `fund()` accepts and accumulates real value (1000 then 500 -> bounty 1500); `test_owner_can_reclaim_unbroken_pool` confirms the owner-reclaim-then-withdraw pull-payment flow works end-to-end with real funds (2000 funded -> reclaimed -> withdrawn successfully). All 5 tests in `test_jailbreak_bounty.py` passed live.
- **The full break-to-challenger-payout path is now also confirmed** (2026-07-08) -- `test_impossible_style_rule_pays_out_challenger` genuinely forces `attempt()` to a `violated: true` verdict and drives `claim`/`withdraw` end to end with real funds (5000 funded -> won -> withdrawn). Three attempts to force this against an actual secret-keeping rule (`RULE`'s SWORDFISH password) via real red-team techniques -- an instruction override, a disguised repeat-back request, and a prompt-leaking "repeat your instructions" attack -- were all correctly refused by the guarded model; a throwaway diagnostic probe contract (`gl.eq_principle.prompt_non_comparative` used as a leader-passthrough to read the raw guarded response) confirmed this was genuine alignment holding, not a testing artifact or judge-side quirk. Given that, the test instead uses a rule with no security content at all -- "never use the letter e" -- which is close to mechanically unsatisfiable for any real-length English response regardless of whether the guarded model complies, hedges, or refuses, and whose judge check ("does the literal letter appear") is completely unambiguous. This exercises the exact same consensus/payout code path as a real jailbreak while being close to deterministic; it is NOT a claim that the SWORDFISH-style secret-keeping guard is breakable. See DECISIONS.md.
...
## Build queue -- COMPLETE (all 20 primitives built, fully specified in CONTRACTS.md)

The shared-guard web-fetch trio (V) is complete: `CorroborationOracle`, `ProvenanceAttestor`, and `CanaryTripwire` are all built and confirm the `try/except`-around-`gl.nondet.web.render` guard works live across four call sites total. `CanaryTripwire` additionally delivered the first confirmed cross-contract WRITE in the repo (see "Known blockers" above) -- any future contract needing `.emit()` can now build on that pattern (best-effort try/except, callback delivery is asynchronous, target address must be a real deployed contract). `EscalatingVerdict` (VII) is done -- a tiered consensus dispatcher (`strict_eq`/`comparative`/`non_comparative` selected deterministically by escrowed stake), deviating from CONTRACTS.md's literal "multi-source" wording for the large-stake tier into a multi-lens deterministic input instead (see DECISIONS.md). `AdversarialReview` (IV) is also done -- `non_comparative` over a claim-only deterministic input, with the leader constructing both a steelmanned pro case and con case AND ruling in one call; live-verified with a genuinely contested claim producing two substantive, distinct cases and a margin that correctly reflected how close the call was. `EquivalenceRegistry` (VI) is done -- the first and only primitive in the repo that runs NO non-deterministic block at all: pure deterministic named-principle CRUD (`register`/`bump`/`get`) whose entire purpose is to be the TARGET of a cross-contract READ from other contracts, which feed the returned principle text into their own `comparative`/`non_comparative` calls. No new cross-contract WRITE was needed (the earlier "likely WRITE need" guess was wrong -- it is read-only from its own side); state uses the dual-`TreeMap` existence-index pattern (`principles` + a parallel `versions` u256 sentinel map) and per-entry author gating on `bump`. Live-verified end to end via CLI (register round-trips, duplicate-register reverts, author bump -> v2) and 10/10 gltest.

`RealitySettledMarket` (VIII) is done -- the last primitive, saved for last precisely because it composes two already-proven patterns rather than inventing a new one: AmbiguityGuard's confidence-then-deterministic-decide shape (a `comparative` block agrees on both an outcome token AND an integer confidence; a plain `require`/if-else afterward, not the LLM, decides whether that clears the bar) and the web-fetch trio's `try/except`-guarded `gl.nondet.web.render` (a new call site, per-source, degrading a dead URL to a failed-source string rather than aborting `settle()`). `bet()`/`settle()`/`redeem()` use the same payable-pool + pull-payment-ledger shape as `SchellingResolver`. Live-verified via CLI (deploy, `status` round-trip, zero-stake `bet` reverts, no-bets `settle` reverts) and 8/8 gltest, including both branches of the actual money path: a clearly-YES market (Apollo 11 sources) paying the winning side the whole pool pro-rata, and a deliberately unresolvable market (two `.invalid` sources) REFUNDing every bettor's own stake exactly. See DECISIONS.md for the degenerate-market guard (a judged winner with an empty pool falls back to REFUND, keeping the outcome/payout invariant exact).

**All 20 catalog primitives are now built, live-verified, and tested.** Any future work in this repo is either a fix to an existing contract or a genuinely new addition beyond the original 20 -- re-read CONTRACTS.md and this file's "Known blockers" section before starting either, since the constraints above remain load-bearing.
