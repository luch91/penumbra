...
## Build queue (remaining 5, fully specified in CONTRACTS.md)

Next up:
- **CanaryTripwire** (V) — the last of the shared-guard web-fetch trio; `CorroborationOracle` and `ProvenanceAttestor` are both done and confirm the `try/except`-around-`gl.nondet.web.render` guard works live at two call sites. Reuse that guard unchanged.
...
Note: CanaryTripwire and RealitySettledMarket both call `gl.nondet.web.*` — wrap every such call in `try/except` inside its nondet closure (see "Known blockers" above, the `NondetException`-on-fetch-failure finding from `SemanticDeadman`, now reconfirmed by `CorroborationOracle` and `ProvenanceAttestor`).
