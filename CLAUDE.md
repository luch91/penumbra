...
Everything below was found by live-deploying contracts to studionet via the CLI across four smoke-test sessions — not just `python3 -m py_compile`. Read this before assuming an untested pattern "should" work; the SDK docs and this file's own prior guidance were wrong on ten separate points below.
