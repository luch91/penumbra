...
## Build queue (remaining 1, fully specified in CONTRACTS.md)
...
Next up:
- **RealitySettledMarket** (VIII), the final primitive, saved for last since it composes both AmbiguityGuard's ambiguity-guard pattern and the web-fetch trio's fetch-guard pattern (see DECISIONS.md's ordering rationale).
