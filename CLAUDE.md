...
Everything below was found by live-deploying contracts to studionet via the CLI across four smoke-test sessions — not just `python3 -m py_compile`. Read this before assuming an untested pattern "should" work; the SDK docs and this file's own prior guidance were wrong on nine separate points below.
...
- **`gltest` has never been run successfully end-to-end against studionet in this repo.** Two bugs were found and fixed while trying (2026-07-01): `gltest.config.yaml`'s `studionet:` key had no value (parsed as YAML `null`, but gltest requires a dict — fixed to `studionet: {}`), and gltest eagerly validates every configured network's required env vars even when a different network is selected. **Still blocking**: every attempted deploy via `gltest` fails with `ValueError: Failed to get schema from all clients (default, hosted studio, and local)`. **This is NOT a funding/gas issue** — studionet does not require GEN for gas (this repo's own `genlayer` CLI account has sat at `0 GEN` through every successful deploy this session, disproving an earlier draft of this note that assumed otherwise). Reading `gltest`'s source (`contract_factory.py::_get_schema_with_fallback`) shows the schema is fetched from the contract's *source code*, independent of any account balance, via three separately-configured clients ("default", "hosted studio", "local") — all three failed here, pointing to a client-connectivity/configuration gap in this shell, not an unfunded-account gap. Root cause still unconfirmed. Every test file in this repo (`tests/test_*.py`) is believed correct by inspection and mirrors CLI-verified behavior, but has not itself been executed successfully. See `docs/SETUP.md`'s "Running the test suite" section.
