...
Verified with real `gltest` runs, not just `py_compile`, across all six contracts (2026-07-02 sweep): `DissensusOracle` 4/4, `MirrorAudit` 5/5 + isolation test 1/1, `SemanticDeadman` 6/6, `JailbreakBounty` 5/5 (first confirmation of the payable `fund()` path end-to-end -- `gltest`'s `.transact(value=N)` can send real value where the `genlayer` CLI cannot). `SchellingResolver` (2/7) and `ProofCarryingAnswer` (0/4 clean) hit session-level TLS/connection flakiness against `studio.genlayer.com` (`SSLV3_ALERT_BAD_RECORD_MAC`, `record layer failure`, `RemoteDisconnected`) on their payable-path tests specifically -- confirmed not a code issue (a raw single HTTP request succeeded mid-episode, and other suites ran clean immediately before/after), but left unconfirmed pending a re-run in a future session. See `DECISIONS.md`'s 2026-07-02 entry for the full story.
