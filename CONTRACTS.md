...
The current SemanticDeadman deployment was completed on 2026-08-14 through GenShipyard on `studionet` (`https://studio.genlayer.com/api`), using runner `py-genlayer:1jb45aa8ynh2a9c9xn3b7qqh8sm5q93hwfp7jqmwsfhh8jpz09h6`. The table retains historical entries for the other catalogue contracts. Studio state can reset between sessions, so verify addresses before relying on them.

**Verification status.** All contract and test files pass local Python compilation and whitespace checks. The full `gltest --network studionet tests` suite collected 155 tests and passed all 155 on 2026-08-28. Docker localnet is running, but it has no configured LLM validators. Live Studionet transfer checks are recorded below.
...
**SemanticDeadman redeployment evidence.** The corrected deployment finalized on 2026-08-14 with five validators and majority agreement. [Deployment transaction](https://explorer-studio.genlayer.com/tx/0x8669e0db1a0295e216733027446ec949082636553ba958ee68a8f09a8cf4f1a2) `0x8669e0db1a0295e216733027446ec949082636553ba958ee68a8f09a8cf4f1a2` finalized with `MAJORITY_AGREE`. The deployed contract is `0x56535AC15Ec5925fB8C7eEeD67140a807fF48657`. The verified schema exposes the constructor, `fund`, `check_in`, `poke`, `claim`, `claimable_of`, and `status`. The initial `status()` read reported `released: false`, `treasury: 0`, and an empty `last_alive_snapshot`.

**Remaining catalogue SDK checks.** The eleven previously historical catalogue deployments were exercised live through the JavaScript SDK. Accepted representative workflows were: ProofCarryingAnswer attestation (`0xc5089126...`), MirrorAudit cross-contract audit (`0x2b6a419a...`), PolyglotConsensus submission (`0x6cf5c187...`), SemanticCommitReveal commit, phase change, and reveal (`0xbb913cc8...`, `0xf299405a...`, `0x1ed8d60e3...`), IntentLock request (`0xb8c3b5dd...`), SemanticDiffLedger proposal (`0x26297752...`), ProvenanceAttestor source attestation (`0x8cb22e259...`), CanaryTripwire arm and poll (`0x21a9a600...`, `0xe78b787e...`), AdversarialReview adjudication (`0x3fd8eb1f...`), EquivalenceRegistry registration (`0x0945bbb8...`), and ConstitutionalContract amendment proposal (`0x3bc819ca...`). All listed transactions reached `ACCEPTED` or `FINALIZED`; the corresponding state reads confirmed the expected archive, phase, verdict, routing, or tripwire transitions.
...
| 5 | SemanticDeadman | [`0x56535AC15Ec5925fB8C7eEeD67140a807fF48657`](https://explorer-studio.genlayer.com/address/0x56535AC15Ec5925fB8C7eEeD67140a807fF48657) | [`0x8669e0db1a0295e216733027446ec949082636553ba958ee68a8f09a8cf4f1a2`](https://explorer-studio.genlayer.com/tx/0x8669e0db1a0295e216733027446ec949082636553ba958ee68a8f09a8cf4f1a2) | `beneficiary=0x9dd54bb14f41701b1734205f66cc8e99e59649f5, liveness_url="https://this-domain-genuinely-does-not-exist-penumbra-test-8842.com/", liveness_policy="Alive only if the page loads and shows a recent, dated public post."` |
...
### 1. DissensusOracle [x] `contracts/dissensus_oracle.py`
...
### 2. AmbiguityGuard [x] `contracts/ambiguity_guard.py`
...
### 3. ProofCarryingAnswer [x] `contracts/proof_carrying_answer.py`
...
### 4. PolyglotConsensus [x] `contracts/polyglot_consensus.py`
...
### 5. SemanticCommitReveal [x] `contracts/semantic_commit_reveal.py`
...
### 6. IntentLock [x] `contracts/intent_lock.py`
...
### 7. SemanticDiffLedger [x] `contracts/semantic_diff_ledger.py`
...
### 8. ConstitutionalContract [x] `contracts/constitutional_contract.py`
...
### 9. JailbreakBounty [x] `contracts/jailbreak_bounty.py`
...
### 10. SchellingResolver [x] `contracts/schelling_resolver.py`
...
### 11. AdversarialReview [x] `contracts/adversarial_review.py`
...
### 12. CorroborationOracle [x] `contracts/corroboration_oracle.py`
...
### 13. ProvenanceAttestor [x] `contracts/provenance_attestor.py`
...
### 14. CanaryTripwire [x] `contracts/canary_tripwire.py`
...
### 15. ConsensusThermometer [x] `contracts/consensus_thermometer.py`
...
### 16. MirrorAudit [x] `contracts/mirror_audit.py`
...
### 17. EquivalenceRegistry [x] `contracts/equivalence_registry.py`
...
### 18. SemanticDeadman [x] `contracts/semantic_deadman.py`
...
### 19. EscalatingVerdict [x] `contracts/escalating_verdict.py`
...
### 20. RealitySettledMarket [x] `contracts/reality_settled_market.py`
